...
export const fulfillPaidTransaction = async (
    ...
    try {
        ...
        if (employee?.name && employee.periodStart && employee.periodEnd) {
            try {
        ...
        if (updateError) {
            console.error('DB update setelah email gagal:', updateError);
        }

        console.log(`✅ PDF dikirim ke ${row.customer_email} (${externalId})`);
        return { ok: true, message: `PDF dikirim ke ${row.customer_email}` };
    } catch (error: any) {
        console.error('fulfillPaidTransaction error:', error);
        return {
            ok: false,
            message: error.message || 'Gagal membuat PDF atau mengirim email',
        };
    }
};
